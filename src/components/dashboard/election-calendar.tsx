    ...
    return (
      <div className="glass-card p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-[#1a1a2e]">Election Calendar</h2>
          <Link href="/elections/new" className="text-xs text-blue-500 hover:underline">+ Add</Link>
        </div>
        <div className="text-center py-6 text-zinc-400 text-xs">
          No elections configured. <Link href="/elections/new" className="text-blue-500 hover:underline">Add one</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="glass-card p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href="/elections" className="text-xs text-zinc-400 hover:text-zinc-700 transition-colors">
          View all →
        </Link>
      </div>
      ...
      <div className="space-y-2.5">
        {elections.map(election => {
          ...
          return (
            <div key={election.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${
              isImminent
              ...
              <div className={`w-10 h-10 rounded-lg flex flex-col items-center justify-center shrink-0 ${
                isImminent ? 'bg-red-100' : isActive ? 'bg-orange-100' : isPastElection ? 'bg-zinc-100' : 'bg-blue-50'
              }`}>
                <div className={`text-sm font-bold leading-none ${
                  isImminent ? 'text-red-700' : isActive ? 'text-orange-700' : isPastElection ? 'text-zinc-400' : 'text-blue-700'
                }`}>
                  {format(date, 'd')}
                </div>
                <div className={`text-[9px] uppercase tracking-wider ${
                  isImminent ? 'text-red-500' : isActive ? 'text-orange-500' : isPastElection ? 'text-zinc-300' : 'text-blue-500'
                }`}>
                  {format(date, 'MMM yy')}
                </div>
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="text-xs font-semibold text-zinc-800 truncate">{election.name}</div>
